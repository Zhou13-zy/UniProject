...
export default function UserCardPopupAvatar({user}) {
  ...
  const handleClose = () => setOpen(false);
	...
	const sendRequest = async () => {
		await apiCall('POST', headers, {}, `request?requestTo=${user.emailAddress}&requestFrom=${userId}`);
		alert('Request Sent!');
	}
  ...
  return (
    <div>
			<div size='lg' style={{cursor:'pointer'}} className='mx-1' onClick={handleOpen}>
								...
								<MDBCard style={{ borderRadius: '15px' }}>
									<MDBCardBody className="text-center">
										...
										<MDBCardText className="text-muted mb-4">
											{user.emailAddress} <span className="mx-2">|</span> <a href="#!">Peera</a>
										</MDBCardText>
										{
										...
										<div className="d-flex justify-content-between text-center mt-5 mb-2">
											<div>
												<MDBCardText className="mb-1 h5">{userProjectNum ?? '0'}</MDBCardText>
